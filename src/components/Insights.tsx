...
export default function Insights({
  ...
  }, [apiBase, apiToken, period, customDateFrom, customDateTo, accountIds, categoryIds, tagIds, includeUncategorized, includeUntagged, categories, tags]);
  ...
  return (
    <section className="insights-print-area mx-auto max-w-6xl px-6 py-10">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <button
          type="button"
          className="no-print flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors"
          style={{
        ...
        <div className="space-y-4 min-w-0">
          ...
          <div className="card">
            ...
            {!cardCollapsed.byCategory && (
              <div className="mt-4">
                {insightsData.by_category.length === 0 ? (
                  <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>
                    {t.insightsEmpty}
                  </p>
                ) : (() => {
                  const sorted = [...insightsData.by_category].sort((a, b) => b.total - a.total);
                  const CATEGORY_CHART_W = 900;
                  const categoryChartH = Math.min(
                    CATEGORY_CHART_MAX_HEIGHT,
                    Math.max(CHART_HEIGHT, sorted.length * CATEGORY_BAR_HEIGHT)
                  );
                    ...
                    >
                      ...
                      >
                          ...
                          <Tooltip
                            ...
                            formatter={(value: number) => [formatAmount(value, "EUR", locale), t.insightsAmount]}
                          />
                    ...
                    <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
                      ...
                      >
                        ...
                        <Tooltip
                          ...
                          labelStyle={{ color: "var(--text)" }}
                          itemStyle={{ color: "var(--text)" }}
                          formatter={(value: number) => [formatAmount(value, "EUR", locale), t.insightsBalance]}
          ...
          <div className="card">
            ...
            >
              <h3 className="card-title mb-0">{t.insightsCardBalanceComparison}</h3>
              <i
                className={`fa-solid fa-chevron-${cardCollapsed.balanceComparison ? "down" : "up"}`}
                style={{ color: "var(--text-secondary)" }}
              />
            </button>
